...
if command == "clean":
    ...
    for addon in sorted(dirs):
        ...
        for po in glob.glob(f"{addon}/po/*-local.po"):
            ...
            system(f'msgfmt {po} -o "{addon}/locale/{locale}/LC_MESSAGES/addon.mo"')
        tgz = os.path.join(
            "..", "glopglop-addons", gramps_version, "download", addon + ".addon.tgz"
        )
        patts = [
            f"{addon}/*.py",
            f"{addon}/*.glade",
    ...
    mkdir(f"../glopglop-addons/{gramps_version}/listings")
    for lang in languages:
        output = []
        for plugin in sorted(listings[lang], key=lambda p: (p["t"], p["i"])):
            output.append(plugin)
        with open(
            f"../glopglop-addons/{gramps_version}/listings/" + ("addons-%s.json" % lang),
            "w",
            encoding="utf-8",
            newline="",
        ) as fp_out:
    ...
    for tgz in glob.glob(f"../glopglop-addons/{gramps_version}/download/*.tgz"):
    ...
    fp_in = open(
        f"../glopglop-addons/{gramps_version}/listings/addons-en.json", "r", encoding="utf-8"
    )
    addons = {}
            ...
            for gpr in glob.glob(f"{addon}/*.gpr.py"):
                ...
                for p in plugins:
                    ...
                    tgz_exists = os.path.isfile(
                        f"../glopglop-addons/{gramps_version}/download/" + tgz_file
                    )
